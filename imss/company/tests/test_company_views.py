...
class CompanyListTestCase(APITestCase):
    ...
    #arrumar
    @patch('company.serializers.company_serializer.CompanySerializer')
    def test_post_company_with_invalid_payload(self, mock_company_serializer):
        mock_serializer_instance = mock_company_serializer.return_value
        mock_serializer_instance.is_valid.return_value = False
        mock_serializer_instance.save.return_value = self.invalid_payload
        mock_serializer_instance.errors = {'cnpj': ['This field is required.']}

        self.client.force_authenticate(user=self.user)
        response = self.client.post(self.url_post, data=self.invalid_payload, format='json')
        serializer_errors = mock_serializer_instance.errors
    ...
    #arrumar
    @patch('company.serializers.company_serializer.CompanySerializer')
    def test_invalid_payload(self, mock_company_serializer):
        mock_instance = mock_company_serializer.return_value
        mock_instance.is_valid.return_value = False

        self.client.force_authenticate(user=self.user)
        response = self.client.put(self.url_put, self.invalid_payload, format='json')
